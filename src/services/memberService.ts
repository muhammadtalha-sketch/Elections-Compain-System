...
function toMember(row: Member): FirestoreMember {
  return {
    ...
    gender: (row.gender === 'Male' || row.gender === 'Female') ? row.gender : 'Male',
    dob: row.dob ?? '',
...
export async function getMemberById(id: string): Promise<FirestoreMember | null> {
  const { data, error } = await supabase
    .from('members')
    .select('*')
    .eq('id', id)
...
export async function addMember(
  ...
  const { data: nextSerial, error: serialError } = await supabase.rpc('get_next_serial_number')
  if (serialError) throw new Error(`Could not generate serial number: ${serialError.message}`)
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      dob:                member.dob || null,
      birth_year:         member.dob ? parseInt(member.dob.slice(0, 4)) : (member.birthYear || null),
...
export async function updateMember(id: string, updates: Partial<FirestoreMember>): Promise<void> {
  ...
  if (updates.phoneNumber !== undefined)       patch.phone_number = updates.phoneNumber || null
  if (updates.requestMemberBar !== undefined)  patch.request_member_bar = updates.requestMemberBar || null
  if (updates.registrationDate !== undefined)  patch.registration_date = updates.registrationDate
...
export async function searchMembers(filters: MemberSearchFilters): Promise<FirestoreMember[]> {
  ...
  if (filters.fatherName)        query = query.ilike('father_name', `%${filters.fatherName}%`)
  if (filters.serialFrom !== undefined) query = query.gte('serial_number', filters.serialFrom)
  if (filters.serialTo !== undefined)   query = query.lte('serial_number', filters.serialTo)
...
export async function getDistinctBars(): Promise<string[]> {
  const { data, error } = await supabase
    ...
    .limit(2000)
  if (error) return []
  return [...new Set((data ?? []).map((r) => r.request_member_bar).filter(Boolean) as string[])]
}
